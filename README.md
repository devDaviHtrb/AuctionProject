...
- ### Close Auction ```/sockets/CloseRoom```
  Sempre que um leilão é criado ele tera um tempo pré determinado para que ele seja fechado, entretanto poderia ocorrer de um usuario fazer um lance no ultimo minuto, entao sempre que um usuario faz um lance nos ultimos 2 minutos o tempo volta a ter 2 minutos

  A funcao ```close_auction(id_auction)``` tira todas as pessoas da socket room, apaga o leilão e muda o status dele
