...
def create_user(data: dict):
    ...
    if user_type == "physical_person":
        physical_person = physical_persons(user_id = user.user_id, rg = data.get("rg"), birth_date = data.get("birth_date"), gender = data.get("gender"))
        db.session.add(physical_person)
    else:  # legal person
        legal_person = legal_persons(user_id = user.user_id, scrap_purchase_authorization = data.get("scrap_purchase_authorization", False),trade_name = data.get("trade_name"), legal_business_name = data.get("legal_business_name"),cnpj = data.get("cnpj"), state_tax_registration = data.get("state_tax_registration"))
